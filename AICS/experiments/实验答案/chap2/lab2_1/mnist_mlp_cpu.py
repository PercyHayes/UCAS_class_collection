...
class MNIST_MLP(object):
    ...
    def load_data(self):
        ...
        train_labels = self.load_mnist(os.path.join(MNIST_DIR, TRAIN_LABEL), False)#_______________________________ 
        test_images = self.load_mnist(os.path.join(MNIST_DIR, TEST_DATA), True)#________________________________
        test_labels = self.load_mnist(os.path.join(MNIST_DIR, TEST_LABEL), False)#_________________________________
    ...
    def train(self):
        ...
        for idx_epoch in range(self.max_epoch):
            ...
            for idx_batch in range(max_batch):
                ...
                #pred_labels = np.argmax(prob, axis=1)
                #pred_results[-last_batch:] = pred_labels
                #accuracy = np.mean(pred_results == mlp.batch_labels[:,-1])
                self.backward()
                self.update(self.lr)
                if idx_batch % self.print_iter == 0:
                   print('Epoch %d, iter %d, loss: %.6f' % (idx_epoch, idx_batch, loss))
                   #print('Epoch %d, iter %d, loss: %.6f, accuracy: %.6f' % (idx_epoch, idx_batch, loss,accuracy))
...
def build_mnist_mlp(param_dir='weight.npy',lr=0.01):
    h1,h2,e=512,256,10 #32,16,10
    mlp=MNIST_MLP(hidden1=h1,hidden2=h2,max_epoch=e,lr=lr)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    mlp.train()
