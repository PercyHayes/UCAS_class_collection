...
def test_op():
    ...
    for n in range(N_CTX,N_CTX+1):
        ...
        if(pytorch_valible==True):
            ...
            rel_tp_error = abs_tp_error / torch.max(torch.sum(torch.abs(pyt_out)) + torch.sum(torch.abs(tri_out)), torch.tensor(1e-8, device="mlu"))
            ...
            rel_sp_error = abs_sp_error / torch.max(torch.sum(torch.abs(pyt_out)) + torch.sum(torch.abs(sdpa_output)), torch.tensor(1e-8, device="mlu"))
        ...
        rel_ts_error = abs_ts_error / torch.max(torch.sum(torch.abs(sdpa_output)) + torch.sum(torch.abs(tri_out)), torch.tensor(1e-8, device="mlu"))
