...
import sys
sys.path.append("/opt/code_chap_8_opt_student/codellama")
#/workspace/algorithm/code_chap_8_opt_student/llama3.2
#sys.path.append("/workspace/algorithm/code_chap_8_opt_student/llama3.2")
from flash_attention_triton_opt import attention as triton_attention
...
class MllamaVisionSdpaAttention(MllamaVisionAttention):
    ...
    def forward(
        ...
        attn_output = triton_attention(query, key, value, False, 1/math.sqrt(self.head_dim), attention_mask).half()

        global is_activate
        is_activate += 1
        #if is_activate ==1:
        print("Llama3.2 opt PASS!")
...
class MllamaTextCrossSdpaAttention(MllamaTextCrossAttention):
    ...
    def forward(
        ...
        if query_states.device.type == "mlu" and attention_mask is not None:
...
class MllamaTextSelfSdpaAttention(MllamaTextSelfAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # SDPA with memory-efficient backend is currently (torch==2.1.2) bugged with non-contiguous inputs with custom attn_mask,
        # Reference: https://github.com/pytorch/pytorch/issues/112577.
        if query_states.device.type == "mlu" and causal_mask is not None:
            query_states = query_states.contiguous()
            key_states = key_states.contiguous()
            value_states = value_states.contiguous()
...
class MllamaTextModel(MllamaPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            and attention_mask.device.type == "mlu"
        ...
        **kwargs,
        ...
        if attention_mask is not None and position_ids is None:
            ...
            if past_key_values:
                ...
                # This `clone` call is needed to avoid recapturing mlu graphs with `torch.compile`'s  `mode="reduce-overhead`, as otherwise the input `position_ids` would have various stride during the decoding. Here, simply using `.contiguous()` is not sufficient as in the batch size = 1 case, `position_ids` is already contiguous but with varying stride which retriggers a capture.
